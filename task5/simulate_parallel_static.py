from os.path import join
import sys
from matplotlib import pyplot as plt
import numpy as np
from concurrent.futures import ProcessPoolExecutor
from functools import partial

DATA_SIZE = 512
MAX_ITER = 20_000
ABS_TOL = 1e-4

def load_data(load_dir, bid):
    u = np.zeros((DATA_SIZE + 2, DATA_SIZE + 2))
    u[1:-1, 1:-1] = np.load(join(load_dir, f"{bid}_domain.npy"))
    interior_mask = np.load(join(load_dir, f"{bid}_interior.npy"))
    return u, interior_mask

def summary_stats(u, interior_mask):
    u_interior = u[1:-1, 1:-1][interior_mask]
    mean_temp = u_interior.mean()
    std_temp = u_interior.std()
    pct_above_18 = np.sum(u_interior > 18) / u_interior.size * 100
    pct_below_15 = np.sum(u_interior < 15) / u_interior.size * 100
    return {
        "mean_temp": mean_temp,
        "std_temp": std_temp,
        "pct_above_18": pct_above_18,
        "pct_below_15": pct_below_15,
    }

def visualize(all_u):
    num_matrices = len(all_u)
    fig, axes = plt.subplots(1, num_matrices, figsize=(6 * num_matrices, 6))
    if num_matrices == 1:
        axes = [axes]
    for i, u in enumerate(all_u):
        axes[i].imshow(u, cmap="inferno", origin="lower")
        axes[i].set_title(f"Matrix {i + 1}")
        axes[i].set_xlabel("X")
        axes[i].set_ylabel("Y")
    plt.tight_layout()
    plt.savefig("output/visualization.png")

def check_result_is_close(u0, interior_mask, u):
    og_u = og_jacobi(u0, interior_mask, MAX_ITER, ABS_TOL)
    return np.allclose(og_u, u)

def og_jacobi(u, interior_mask, max_iter, atol=1e-6):
    u = np.copy(u)
    for i in range(max_iter):
        u_new = 0.25 * (u[1:-1, :-2] + u[1:-1, 2:] + u[:-2, 1:-1] + u[2:, 1:-1])
        u_new_interior = u_new[interior_mask]
        delta = np.abs(u[1:-1, 1:-1][interior_mask] - u_new_interior).max()
        u[1:-1, 1:-1][interior_mask] = u_new_interior
        if delta < atol:
            break
    return u

def jacobi(u, interior_mask, max_iter, atol=1e-6):
    u = np.copy(u)
    for i in range(max_iter):
        u_new = 0.25 * (u[1:-1, :-2] + u[1:-1, 2:] + u[:-2, 1:-1] + u[2:, 1:-1])
        u_new_interior = u_new[interior_mask]
        delta = np.abs(u[1:-1, 1:-1][interior_mask] - u_new_interior).max()
        u[1:-1, 1:-1][interior_mask] = u_new_interior
        if delta < atol:
            break
    return u

def simulate_building(bid, load_dir, max_iter, abs_tol):
    u0, interior_mask = load_data(load_dir, bid)
    u = jacobi(u0, interior_mask, max_iter, abs_tol)
    stats = summary_stats(u, interior_mask)
    return bid, u, interior_mask, stats

if __name__ == "__main__":
    LOAD_DIR = "/dtu/projects/02613_2025/data/modified_swiss_dwellings/"
    with open(join(LOAD_DIR, "building_ids.txt"), "r") as f:
        building_ids = f.read().splitlines()

    if len(sys.argv) < 2:
        N = 1
    else:
        N = int(sys.argv[1])
    building_ids = building_ids[:N]

    simulate_one = partial(simulate_building, load_dir=LOAD_DIR, max_iter=MAX_ITER, abs_tol=ABS_TOL)

    num_workers = 4
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(simulate_one, building_ids))

    all_u = np.empty((N, 514, 514))
    all_interior_mask = np.empty((N, 512, 512), dtype=bool)
    for i, (bid, u, interior_mask, _) in enumerate(results):
        all_u[i] = u
        all_interior_mask[i] = interior_mask

    visualize(all_u)

    print("Test")
    print("----------------")
    print("Checking the first result against the original Jacobi method...")
    is_close = check_result_is_close(all_u[0], all_interior_mask[0], all_u[0])
    if not is_close:
        print("Your result is incorrect")
        print("!!!! FIX YOUR CODE BEFORE MOVING ON !!!!")
    else:
        print("Your result is correct")
    print()

    print("Result summary")
    print("----------------")
    stat_keys = ["mean_temp", "std_temp", "pct_above_18", "pct_below_15"]
    print("building_id," + ", ".join(stat_keys))
    for i, (bid, _, _, stats) in enumerate(results):
        print(f"{bid}," + ", ".join(str(stats[k]) for k in stat_keys))
